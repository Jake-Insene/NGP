...
inline T read_at(CPUCore& core, VirtualAddress addr) {
    if (addr + (sizeof(T) + 1) <= BIOS_END && core.current_el == BIOS_REQUESTED_LEVEL) {
        return *reinterpret_cast<T*>(addr + MAPPED_BIOS_ADDRESS);
    }
    else if (addr >= IO_START && addr + sizeof(T) <= IO_START + IO_SIZE && core.current_el > 0) {
        return IO::read_io<T>(core, addr);
    }
    else if (addr >= RAM_START && addr + sizeof(T) <= RAM_END + ram_size) {
...
inline void write_at(CPUCore& core, VirtualAddress addr, T value) {
    ...
    else if (addr >= IO_START && addr + sizeof(T) <= IO_END) {
        IO::write_io<T>(core, addr, value);
    }
    else if (addr >= RAM_START && addr + sizeof(T) <= RAM_START + ram_size) {
